...
### Every year looks the same?! Part 1: Danceability, valence and mode {.storyboard}
...
### How the oldest song and highest new entrant differ: Cepstrograms {.storyboard}
```{r}
Goldband <-
...
### Key-changes by your favourite boyband: Keygrams {.storyboard}
```{r}
Waterboys <-
...
**The boyband of the 90s: The Backstreet Boys**
...
**The band with the most songs: The Beatles**
...
### Happy hardcore or happy hardrock? Tempograms {.storyboard}
```{r}
...
**Happy Hardcore with Rainbow in the Sky**
...
**Headbanging with Killing in the Name**
...
### Every year looks the same?! Part 2: Timbre {.storyboard}
```{r}
Top1999timbre1 <-
...
### Every year looks the same?! Part 3: Loudness, tempo and duration {.storyboard}
```{r}
...
### Tonight we're gonna party like it's 1999 {.storyboard}
```{r}
Top1999timbre <-
  get_playlist_audio_features(
    "thesoundsofspotify",
...
### Tonight we're gonna party like it's 2022
```{r, fig.width=12, fig.height=5}
Dendro2022 <-
...
The chromagrams show that there are different levels of complexity when it comes to the use of chroma. However, the level of complexity doesn't necessarily give the song a better position in the TOP2000. From the chromagrams we are able to distinguish different parts of songs (Fix You), but we are also able to do this if we use cepstrograms (Heart-Shaped Box).  For the cepstrograms I have used songs from different decades. Summertime stands out by showing a high magnitude for c03 which makes me believe that it may be characteristic for songs from the 30s. This is only based on one song and would need to be researched further before making any conclusions. The self-similarity matrices show two examples of songs with a different structure. The keygrams also range from simple with The Whole of the Moon to more complex with Penny Lane. In the tempograms we see a similar pattern, Rainbow In The Sky shows a straight line while Killing In The Name is more dynamic in tempo. In conclusion, the individual song analysis tells us that there is something of everything in the TOP2000 and that there doesn't seem to be a relation between certain patterns in combination with a type of genre or era. 

**Year analysis**

The scatterplot about danceability, valence and mode already shows a very small difference between 1999, 2009, 2019 and 2022. This minimal change is confirmed by the violinplot that is based on the timbre coefficients. Again, in the scatterplot about loudness, tempo and duration the same pattern is visible, that all years portray a similar distribution.

**Final thoughts**

From these results I would argue that the TOP2000 hasn't changed in sound. As a discussion point I want to add that for some plots or grams I only used the first 250 or 100 songs from the lists. The results may change if the entire TOP2000 is analysed, but I don't expect any big differences compared to the current outcome.
