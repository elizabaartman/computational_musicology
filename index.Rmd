...
plot_grid(Bohemian_chromagram, Brel_chromagram, FixYou1_chromagram, Ramses_chromagram, ncol = 1)
```
...
Nirvana_cepstrogram <- Nirvana |>
  ...
  geom_tile() +
  geom_vline(xintercept = 46, colour = "red") +
  geom_vline(xintercept = 87, colour = "red") +
  geom_vline(xintercept = 123, colour = "red") +
...
plot_grid(Goldband_cepstrogram, Summertime_cepstrogram, Nirvana_cepstrogram, DoeMaar_cepstrogram, ncol = 1)
```
...
This is a self-similarity matrix based on ABBA's song Dancing Queen. 
...
Dendro1999_juice <-
  ...
  prep(Dendro1999 |> mutate(track.name = str_trunc(track.name, 20))) |>
  juice() |>
...
Dendro2009_juice <-
  ...
  step_scale(all_predictors()) |> 
  # step_range(all_predictors()) |> 
  prep(Dendro2009 |> mutate(track.name = str_trunc(track.name, 20))) |>
  juice() |>
  column_to_rownames("track.name")
...
Dendro2019 <-
  get_playlist_audio_features("2019", "0qoaHjMRoMN7v9A56R5yYB") |>
  add_audio_analysis() |>
  mutate(
    segments = map2(segments, key, compmus_c_transpose),
    pitches =
...
Dendro2019_juice <-
  ...
  step_scale(all_predictors()) |> 
  # step_range(all_predictors()) |> 
  prep(Dendro2019 |> mutate(track.name = str_trunc(track.name, 20))) |>
  juice() |>
  column_to_rownames("track.name")
...
Dendro2022 <-
  get_playlist_audio_features("2022", "0qoaHjMRoMN7v9A56R5yYB") |>
  add_audio_analysis() |>
  mutate(
    segments = map2(segments, key, compmus_c_transpose),
    pitches =
...
Dendro2022_juice <-
  ...
  step_scale(all_predictors()) |> 
  # step_range(all_predictors()) |> 
  prep(Dendro2022 |> mutate(track.name = str_trunc(track.name, 20))) |>
  juice() |>
  column_to_rownames("track.name")
